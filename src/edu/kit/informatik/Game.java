...
public class Game {
    ...
    public boolean isReachable(Playingfigure figure, Gamefield where) {
        ...
        if (targetField.isOccupied() && !figure.getOwner().isApolloMove()) {
            throw new IllegalArgumentException("ERROR: You can't move to "
                    + "an occupied field without apolloMove");
        }

        if (!isClose(currentField, targetField)) {
            throw new IllegalArgumentException("ERROR: You can't move to a field"
                    + " that is not adjecent to you without hermesMove");
        }


        if (figure.getOwner().isHermesTeleport()) return
                targetField.getHeightSquares() == currentField.getHeightSquares();


        return targetField.getHeightSquares() - 1 <= currentField.getHeightSquares()
                && !(targetField.getHeightSquares() > currentField.getHeightSquares()
                && figure.getOwner().isAthenaBlocked());
    }
